...
public class WebColorFieldSkin extends InputFieldSkin {
	...
	public WebColorFieldSkin(final WebColorField control) {
		...
		// in the TextField. The only time this is not the case is when the
		// update
		// to the control happened as a result of an update in the text
		// textField.
		control.valueProperty().addListener(
				integerFieldValueListener = new InvalidationListener() {
	...
	public void dispose() {
		...
		((WebColorField) control).valueProperty().removeListener(
	...
	protected void updateValue() {
		...
		String text = getTextField().getText() == null ? "" : getTextField()
