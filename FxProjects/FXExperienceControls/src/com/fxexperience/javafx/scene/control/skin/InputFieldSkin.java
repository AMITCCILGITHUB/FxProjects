package src.com.fxexperience.javafx.scene.control.skin;
...
import src.com.fxexperience.javafx.scene.control.InputField;
...
public abstract class InputFieldSkin implements Skin<InputField> {
	...
	public InputFieldSkin(final InputField control) {
		...
		// Create the TextField that we are going to use to represent this
		// InputFieldSkin.
		// The textField restricts input so that only valid digits that
		// contribute to the
		// Money can be input.
		textField = new InnerTextField() {
			...
			public void replaceText(int start, int end, String text) {
				...
				String t = textField.getText() == null ? "" : textField
						.getText();
			...
			public void replaceSelection(String text) {
				...
				String t = textField.getText() == null ? "" : textField
						.getText();
				int start = Math.min(textField.getAnchor(),
						textField.getCaretPosition());
				int end = Math.max(textField.getAnchor(),
						textField.getCaretPosition());
				t = t.substring(0, start) + text + t.substring(end);
				if (accept(t)) {
		...
		control.getStyleClass().addListener(
				InputFieldStyleClassListener = new InvalidationListener() {

					@Override
					public void invalidated(Observable observable) {

						textField.getStyleClass().setAll(
		...
		textField.prefColumnCountProperty().bind(
				control.prefColumnCountProperty());
		...
		// we will just make sure the TextField gets focus whenever somebody
		// tries
		...
		control.focusedProperty().addListener(
				InputFieldFocusListener = new InvalidationListener() {
		...
		textField.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				// Because TextFieldBehavior fires an action event on the parent
				// of the TextField
				// (maybe a misfeature?) I don't need to do this. But I think
				// this is
