...
public class SplineEditorControl extends XYChart<Number, Number> {
	...
	private Circle controlPoint1Circle = CircleBuilder.create()
			.fill(Color.WHITE).stroke(Color.RED).strokeWidth(3).radius(6)
			.cursor(Cursor.HAND).build();
	private Circle controlPoint2Circle = CircleBuilder.create()
			.fill(Color.WHITE).stroke(Color.RED).strokeWidth(3).radius(6)
			.cursor(Cursor.HAND).build();
	private Line cp1Line = LineBuilder.create().stroke(Color.RED)
			.strokeWidth(2).build();
	private Line cp2Line = LineBuilder.create().stroke(Color.RED)
			.strokeWidth(2).build();
	private Path dottedLinesPath = PathBuilder.create().stroke(Color.WHITE)
			.strokeWidth(1).strokeDashArray(1d, 8d).build();
	private Path splinePath = PathBuilder.create().stroke(Color.WHITE)
			.strokeWidth(2).build();
	...
	public SplineEditorControl() {
		...
		getPlotChildren().addAll(splinePath, dottedLinesPath, cp1Line, cp2Line,
				controlPoint1Circle, controlPoint2Circle);
		...
		controlPoint1Circle.setOnMouseDragged(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				final double x = event.getX()
						+ controlPoint1Circle.getLayoutX();
				final double y = event.getY()
						+ controlPoint1Circle.getLayoutY();
				final double dataX = getXAxis().getValueForDisplay(x)
						.doubleValue();
				final double dataY = getYAxis().getValueForDisplay(y)
						.doubleValue();
		...
		controlPoint2Circle.setOnMouseDragged(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				final double x = event.getX()
						+ controlPoint2Circle.getLayoutX();
				final double y = event.getY()
						+ controlPoint2Circle.getLayoutY();
				final double dataX = getXAxis().getValueForDisplay(x)
						.doubleValue();
				final double dataY = getYAxis().getValueForDisplay(y)
						.doubleValue();
	...
	protected void layoutPlotChildren() {
		...
		splinePath.getElements().setAll(new MoveTo(minx, miny),
	...
	@Override
	protected void dataItemAdded(Series<Number, Number> series, int i,
			Data<Number, Number> data) {

	}

	@Override
	protected void dataItemRemoved(Data<Number, Number> data,
